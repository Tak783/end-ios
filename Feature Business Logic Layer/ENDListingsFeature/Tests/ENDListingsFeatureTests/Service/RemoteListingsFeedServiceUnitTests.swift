...
final class RemoteListingsFeedServiceUnitTests: RemoteListingsFeedServiceTests {
    ...
    func test_load_onSuccess_returnsListings() {
        ...
        var returnedListings: [ENDProductModel]?
        let exp = expectation(description: "Wait for load completion")
        sut.load { result in
            switch result {
        ...
        let expectedFeed = try! decoder.decode(RemoteListingsFeed.self, from: data)
        client.complete(withStatusCode: 200, data: data)
        wait(for: [exp], timeout: 1.0)
        
        XCTAssertEqual(returnedListings, expectedFeed.products.toModels())
    }
...
// MARK: - Make Sut
extension RemoteListingsFeedServiceUnitTests {
